...
import { AsyncStorage, NetInfo, ToastAndroid, Alert } from 'react-native';
...
class Store {
  ...
  getPosts(topic) {
    ...
    let self = this;
    this.getAuthToken().then(function() {
      var url =
        'https://api.producthunt.com/v1/posts/all?search[topic]=' + topic;
      var requestObj = {
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
